...
The executable still writes format-2 JSON through `src/main.rs`; its Cargo dependencies do not include `rgit-store` or `rgit-graph`. New identifiers now retain full UUID suffixes ([PR #8](https://github.com/A-Ricemusic/rethinking-git/pull/8)); existing short IDs remain readable. This reduces accidental collision risk but does not replace immutable no-replace storage or transactions.
