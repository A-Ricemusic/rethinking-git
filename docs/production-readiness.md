...
| High | Object IDs were joined directly into filesystem paths. Named keys could alias: `team/alice` and `team__alice` wrote the same actor grants. Stored identities were not checked. | PR #2: typed ID validation, portable named keys, and stored/requested identity checks. Six CLI regression scenarios. |
