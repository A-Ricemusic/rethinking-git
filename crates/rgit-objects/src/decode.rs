...
fn validate_fields(kind: ObjectKind, map: &[(u64, Value)]) -> Result<(), DecodeObjectError> {
    ...
    if kind != ObjectKind::Policy || map.iter().any(|(k, _)| *k == 2) {
    ...
    match kind {
        ...
        ObjectKind::Blob => {
            ...
            if let Some(inline) = map.iter().find(|(k, _)| *k == 4).map(|(_, v)| v) {
                ...
                let mut total = 0_u64;
                for chunk in array(field(map, 5)?, 5)? {
                    let chunk = exact_map(chunk, &[0, 1], &[], 5)?;
                    oid(field(chunk, 0)?, 5)?;
                    total = total
                        .checked_add(unsigned(field(chunk, 1)?, 5)?)
                ...
                unsigned(field(profile, 0)?, 7)?;
                unsigned(field(profile, 1)?, 7)?;
...
fn references(d: &DecodedObject) -> Result<Vec<ReferenceEdge>, DecodeObjectError> {
    ...
    match d.kind {
        ...
        ObjectKind::Release => {
            ...
            add!(7, ReferenceRole::ReleaseProjectionRules, None);
            add!(
                8,
        ...
        ObjectKind::Policy => {
            ...
            add!(12, ReferenceRole::PolicyKeyEnvelopeSet, None);
        ...
        ObjectKind::Operation => {
            ...
            add!(10, ReferenceRole::OperationPublicEnvelope, None);
            add!(11, ReferenceRole::OperationPrivatePayload, None);
        }
        _ => {}
    }
